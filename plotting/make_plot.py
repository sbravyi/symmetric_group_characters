...
n_min = 10
result_mps = [
    result_mps[i] for i in range(num_trials) if np.sum(result_mps[i]["Mu"]) >= n_min
]
result_sage = [
    result_sage[i] for i in range(num_trials) if np.sum(result_sage[i]["Mu"]) >= n_min
]
num_trials = len(result_mps)
...
data = {}
data["MPS"] = [
    result_mps[i]["MPS_runtime_part1"] + result_mps[i]["MPS_runtime_part2"]
